...
from params import *
from utils import triplet_image_dataset
import torch.nn.functional as F
import json

criterion = nn.CrossEntropyLoss().cuda() if use_gpu else nn.CrossEntropyLoss()
triplet_loss = nn.TripletMarginLoss(margin=triplet_margin, p=2).cuda() if use_gpu else nn.TripletMarginLoss(margin=triplet_margin, p=2)
...
def _each_epoch(mode, loader, model, criterion, optimizer=None, epoch=None):
    ...
    for i, (input, img_p, img_n, target) in enumerate(loader):
        data_time.update(time.time() - end)

        if use_gpu:
            target = target.cuda(async=True)
        input_var = torch.autograd.Variable(input, volatile=(mode != 'train'))
        img_p_var = torch.autograd.Variable(img_p, volatile=(mode != 'train'))
        img_n_var = torch.autograd.Variable(img_n, volatile=(mode != 'train'))
        target_var = torch.autograd.Variable(target, volatile=(mode != 'train'))

        # compute output，只用a的logits计算softmax
        output, embedded_a, embedded_p, embedded_n = model(input_var, img_p_var, img_n_var)
        ...
        loss_apn = triplet_loss(embedded_a, embedded_p, embedded_n)
        distance_ap = F.pairwise_distance(embedded_a, embedded_p, 2)
        distance_an = F.pairwise_distance(embedded_a, embedded_n, 2)
        loss += triplet_weight * loss_apn  #用于BP
        ...
        losses_apn.update(loss_apn.data[0], input.size(0))
        distances_ap.update(distance_ap.mean().data[0], input.size(0))
        distances_an.update(distance_an.mean().data[0], input.size(0)) 
        ...
        if triplet_debug and i % triplet_print_freq == 0:
            print('loss_apn {loss_apn.val:.4f} distance_ap {distance_ap.val:.3f} distance_an {distance_an.val:.3f}'
                  .format(loss_apn = losses_apn, distance_ap = distances_ap, distance_an = distances_an))
    ...
    print('loss_apn {loss_apn.avg:.4f} distance_ap {distance_ap.avg:.3f} distance_an {distance_an.avg:.3f}'
          .format(loss_apn = losses_apn, distance_ap = distances_ap, distance_an = distances_an))
